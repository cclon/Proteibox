...
static const char g_DLL_ExportName[ 0x20 ] = "Test" ;
...
--*/
{
	...
	PIMAGE_NT_HEADERS64		pinhx64					= NULL	; 
	...
	if ( NULL == _pNode || 0 == ImageBaseAddress || NULL == szDllPath )
	{
	...
	//
	// 1.2 验证DLL路径是否存在,不能注入一个无效的DLL路径!
	//
	...
	pinhx64 = (PIMAGE_NT_HEADERS64)( (PCHAR)ImageBaseAddress + ImageBaseAddress->e_lfanew );
	...
	case 0x20B : // x64
		{
			pidd = pinhx64->OptionalHeader.DataDirectory ;
			NumberOfRvaAndSizes = pinhx64->OptionalHeader.NumberOfRvaAndSizes ;
			bIsX64 = TRUE ;
		}
		break ;
	...
	if ( NULL == pNew )
	{
		dprintf( "error! | InjectDll_by_reconstruct_pe(); | 0 == pNew; \n" );
	...
	pNode->XImageNotifyDLL.BeCoveredOldData = kmalloc( out_cbNew );
	if ( NULL == pNode->XImageNotifyDLL.BeCoveredOldData )
	{
		dprintf( "error! | InjectDll_by_reconstruct_pe(); | 为原始数据分配备份的内存块失败 \n" );
		status = STATUS_UNSUCCESSFUL ;
		goto _end_ ;
	}

	memcpy( pNode->XImageNotifyDLL.BeCoveredOldData, pbNewIid, out_cbNew ); // 将原始数据备份至此
	pNode->XImageNotifyDLL.BeCoveredAddr = pbNewIid	 ; // 保存待修改的PE块起始地址,供日后回写用
	pNode->XImageNotifyDLL.BeCoveredSize = out_cbNew ; // 保存待修改的PE块数据大小,供日后回写用
	...
	if( FALSE == Make_address_writable( (PVOID)pidd, NumberOfBytesToProtect, PAGE_READWRITE, &OldAccessProtection ) )
	{
		status = STATUS_UNSUCCESSFUL ;
		goto _end_ ;
	}
	...
	if ( NumberOfRvaAndSizes <= 0xC || 0 == pidd[ IMAGE_DIRECTORY_ENTRY_IAT ].VirtualAddress )
	{
		while ( 1 )
		{
	...
	Make_address_writable( (PVOID)pidd, NumberOfBytesToProtect, OldAccessProtection, NULL ); // 恢复至原有的读写权限

	//  
	// 4. 用重构的PE块填充原来的PE块,达到内核中注入指定DLL到指定进程的效果
	//  

	if( FALSE == Make_address_writable( pbNewIid, out_cbNew, PAGE_READWRITE, &pNew->Stub.OldProtection ) )
	{
		status = STATUS_UNSUCCESSFUL ;
		goto _end_ ;
	}
...
--*/
{
	...
	PCHAR pValidPath, SpaceforDllPath, DllName ;
	...
	if ( NULL == szDllPath )
	{
		dprintf( "error! | InjectDll_by_reconstruct_pe_dep(); | Invalid Paramaters; failed! \n" );
		return  NULL;
	}
	...
	//  
	// 3. 遍历导入表中所有数组,计算数组的总个数(每个单元代表一个引入的DLL); 数组上限为0xC8
	//  
	...
	DllPathLength = strlen( szDllPath );
	cbNew = DllPathLength + 0x14 * NumberOfDll + 0xA8 ; // 新的内存大小 = Sbiedll.dll路径长度 + 当前所有IAT表的总大小 + 0xA8
	...
	RtlZeroMemory( (PVOID)lpBuffer, cbNew );
	...
	//  
	// 7.1 从结构体+0x7C开始,存储所有的IAT单元,除了以下3个DLL对应的IAT
	//  
	...
	SpaceforDllPath = (PCHAR) ((ULONG)lpBuffer + sizeof(NEW_PE_IAT_INFO_before) + 0x14 * nCounts) ;
	pValidPath = (PCHAR) szDllPath;
	if ( 0 == strncmp( szDllPath, "\\??\\", 4 ) ) { pValidPath += 4; }

	strcpy( SpaceforDllPath, pValidPath ); // 拷贝DLL全路径至此 "C:\sudami.dll"

	lpBuffer->Stub.IIN1.Hint = 0; 
	strcpy( lpBuffer->Stub.IIN1.Name, g_DLL_ExportName ); // sudami.dll的导出函数
	...
	//
	// 8. 新增的一个IAT ( 偏移量+0x68~+0x7c)  (size==0x14)
	//
	...
	lpBuffer->IATNew.Name				= pbNewIid_offset + (ULONG)SpaceforDllPath - (ULONG)lpBuffer ;
...
--*/
{
	...
	if ( NULL == pSectionHeader || NULL == pImageDataDirectory )
	{
		dprintf( "error! | FindNearBase(); | Invalid Paramaters; failed! \n" );
		return 0 ;
	}
	...
	if ( pider_start && ResourceDirectorySize )
	{
		...
		ResourceDirectorySize_new = 0xFFFFF000 * ResourceDirectorySize_High8bit + ResourceDirectorySize ;
...
_get_special_section_addr_B_next_ :
	...
	{
		//  
		// 3.1 若当前节的属性 不包含 "可执行块 && 代码块",继续循环,直到找到包含的
		// 
...
--*/
{
	...
	//
	// 1. 由于调用此函数前,已调用Handler_SSDT()得到了ZwProtectVirtualMemory的地址,故下面的情况99%会成功返回 
	//

	bRet = kgetaddrSDT( ZwProtectVirtualMemory );
	if ( TRUE == bRet ){ return TRUE ; }

	//
	// 2. 到这里的几率几乎为0,强制得到ZwProtectVirtualMemory的地址
	//
	...
	{
		dprintf( "error! | Get_ZwProtectVirtualMemory_addr() - FindUnresolved(); NULL == g_ZwProtectVirtualMemory_addr; failed! \n" );
		return FALSE ;
	}
...
--*/
{
	...
	if ( OldAccessProtection ) { *OldAccessProtection = RetValue ; }
	return TRUE;
}
