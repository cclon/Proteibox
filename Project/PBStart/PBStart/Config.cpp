...
BOOL __gbIniAdditional = FALSE; // 是否有必要关注 Templates.ini配置文件(默认不关注),
...
{	
	...
	if ( FALSE == bRet )
	{
		MYTRACE( "error! | CConfigEx() - InitData(); | 构造函数初始化失败! \n" );
	}
}
...
BOOL CConfigEx::InitData()
{
	...
	// 判断文件是否存在
    bRet = PathFileExists( m_szProcteiBoxMainIni );
	if ( FALSE == bRet )
	{
        return FALSE;
	}
	
	if ( __gbIniAdditional ) 
	{ 
		__InitData( m_szProcteiBoxTemplateIni, CONFIG_FILE_NAME_TMPL );
		// 判断文件是否存在
		bRet = PathFileExists( m_szProcteiBoxTemplateIni );
		if ( FALSE == bRet )
		{
			MYTRACE( " \n" );
			return FALSE;
	...
	// 2. 创建事件对象和工作线程
	m_hEvent_InitConfigData_wakeup_R0 = CreateEvent( NULL, FALSE, FALSE, m_szEvent_InitConfigData_wakeup_R0 );
	...
	m_hEvent_InitConfigData_wakeup_R3 = CreateEvent( NULL, FALSE, FALSE, m_szEvent_InitConfigData_wakeup_R3 );
	...
	m_hEvent_waitfor_r0 = CreateEvent( NULL, FALSE, FALSE, m_szEvent_waitfor_r0 );
	...
	m_hEvent_wakeup_r0 = CreateEvent( NULL, FALSE, FALSE, m_szEvent_wakeup_r0 );
	...
	if ( FALSE == bRet )
	{
		MYTRACE( "error! | CConfigEx::InitData() - _StartThread(); | 创建工作线程失败\n" );
		return FALSE ;
	}
...
VOID CConfigEx::__InitData( LPSTR lpdest, LPSTR lpsrc )
{
	...
	GetModuleFileName( NULL, szFileName, ARRSIZEOF(szFileName) );
	PathRemoveFileSpec( szFileName );
	PathAppend( szFileName, lpsrc );
	...
	while ( pTempSec != NULL )
	{
		...
		while (pTempKey != NULL)
		{
			...
			while ( pNode )
			{
				...
				nReturn = m_drv_ProteinBoxDrv->IoControl(
					IOCTL_PROTEINBOX,
					(PVOID)&Buffer,
...
void CConfigEx::_ThreadProc()
{
	...
	while (TRUE)
    {
		MYTRACE( "工作线程: I'm always waiting for you ... \n" );
		...
		MYTRACE( "工作线程: I Had got R0 message. \n" );
